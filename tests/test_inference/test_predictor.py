"""Tests del ensamblador de predicciones (src.inference.predictor)."""
...
class _FakeSeqModel:
    """Modelo de prueba: ignora la entrada y devuelve una salida fija [N,H,3]."""
    ...
    def to(self, device):  # noqa: D401 - paridad con nn.Module
        return self
...
def _make_graph(timestamp):
    ...
    # y: [N=3, H=2, C=3]; canal 0 = actual arr delay, canal 2 = pct target.
    y = torch.zeros(3, 2, 3)
...
def test_predict_to_frame_schema_and_values():
    ...
    # 2 nodos activos x 2 horizontes x 2 secuencias = 8 filas; CCC excluido.
    assert len(df) == 8
...
def test_predict_to_frame_reference_ts_override():
    model = _FakeSeqModel(_fake_out())  # salida con 2 horizontes
    ...
    # reference_ts sustituye el timestamp del grafo en todas las filas.
    assert (df["prediction_ts"] == "2020-01-01T00:00:00").all()
    assert set(df["target_ts"]) == {"2020-01-01T01:00:00", "2020-01-01T04:00:00"}
    assert list(df.columns) == PREDICTION_COLUMNS  # sin columnas de actuals
